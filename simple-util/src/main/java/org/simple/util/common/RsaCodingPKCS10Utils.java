...
public final class RsaCodingPKCS10Utils {
	static final Log log = LogFactory.getLog(RsaCodingPKCS10Utils.class);

	public static String encryptByPubCerFile(String src, String pubCerPath) {
		PublicKey publicKey = RsaReadPKCS10Utils.getPublicKeyFromFile(pubCerPath);
		if (publicKey == null) {
			return null;
		}
	...
	public static String encryptByPubCerText(String src, String pubKeyText) {
		PublicKey publicKey = RsaReadPKCS10Utils.getPublicKeyByText(pubKeyText);
		if (publicKey == null) {
			return null;
		}
		return encryptByPublicKey(src, publicKey);
	...
	public static String decryptByPriPfxFile(String src, String pfxPath, String priKeyPass) {
		...
		PrivateKey privateKey = RsaReadPKCS10Utils.getPrivateKeyFromFile(pfxPath, priKeyPass);
		if (privateKey == null) {
			return null;
		}
	...
	public static String decryptByPriPfxStream(String src, byte[] pfxBytes, String priKeyPass) {
		...
		PrivateKey privateKey = RsaReadPKCS10Utils.getPrivateKeyByStream(pfxBytes, priKeyPass);
		if (privateKey == null) {
			return null;
		}
		return decryptByPrivateKey(src, privateKey);
	...
	public static String decryptByPrivateKey(String src, String privateKey) {
		try {
			PrivateKey priKey = RsaReadPKCS10Utils.getPrivateKey(privateKey);
	...
	public static String encryptByPrivateKey(String src, String privateKey) {
		PrivateKey piKey;
		try {
			piKey = RsaReadPKCS10Utils.getPrivateKey(privateKey);
	...
	public static String decryptByPublicKey(String src, String publicKey) {
		try {
			PublicKey pubKey = RsaReadPKCS10Utils.getPublicKey(publicKey);
